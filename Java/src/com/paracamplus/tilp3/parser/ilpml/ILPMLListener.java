...
import antlr4.TypedILPMLgrammar3Listener;
import antlr4.TypedILPMLgrammar3Parser.ExprContext;
import antlr4.TypedILPMLgrammar3Parser.Type_Context;
import antlr4.TypedILPMLgrammar3Parser.AlternativeContext;
import antlr4.TypedILPMLgrammar3Parser.BinaryContext;
import antlr4.TypedILPMLgrammar3Parser.BindingContext;
import antlr4.TypedILPMLgrammar3Parser.CodefinitionsContext;
import antlr4.TypedILPMLgrammar3Parser.ConstFalseContext;
import antlr4.TypedILPMLgrammar3Parser.ConstFloatContext;
import antlr4.TypedILPMLgrammar3Parser.ConstIntegerContext;
import antlr4.TypedILPMLgrammar3Parser.ConstStringContext;
import antlr4.TypedILPMLgrammar3Parser.ConstTrueContext;
import antlr4.TypedILPMLgrammar3Parser.FuntionTypeContext;
import antlr4.TypedILPMLgrammar3Parser.GlobalFunDefContext;
import antlr4.TypedILPMLgrammar3Parser.InvocationContext;
import antlr4.TypedILPMLgrammar3Parser.LambdaContext;
import antlr4.TypedILPMLgrammar3Parser.LocalFunDefContext;
import antlr4.TypedILPMLgrammar3Parser.LoopContext;
import antlr4.TypedILPMLgrammar3Parser.ProgContext;
import antlr4.TypedILPMLgrammar3Parser.SequenceContext;
import antlr4.TypedILPMLgrammar3Parser.TypeContext;
import antlr4.TypedILPMLgrammar3Parser.UnaryContext;
import antlr4.TypedILPMLgrammar3Parser.VariableAssignContext;
import antlr4.TypedILPMLgrammar3Parser.VariableContext;

public class ILPMLListener implements TypedILPMLgrammar3Listener {
	...
	public void exitLocalFunDef(LocalFunDefContext ctx) {
		...
		IFunctionType funType = new FunctionType(ctx.exitType.node, argsType);
		ctx.node = factory.newTypedNamedLambda(
				factory.newTypedVariable(ctx.name.getText(), funType),
				toVariables(ctx.vars, argsType), ctx.body.node, funType);
	}

	@Override
	public void exitCodefinitions(CodefinitionsContext ctx) {
		ctx.node = factory.newCodefinitions(toTypedNamedLambdas(ctx.defs),
				ctx.body.node);
	}
	...
	public void exitLambda(LambdaContext ctx) {
		...
		IFunctionType funType = new FunctionType(ctx.exitType.node, argsType);
		ctx.node = factory.newTypedLambda(toVariables(ctx.vars, argsType),
				ctx.body.node, funType);
	}

	protected IASTtypedNamedLambda[] toTypedNamedLambdas(
			List<LocalFunDefContext> ctxs) {
		if (ctxs == null)
			return new IASTtypedNamedLambda[0];
		IASTtypedNamedLambda[] r = new IASTtypedNamedLambda[ctxs.size()];
		int pos = 0;
		for (LocalFunDefContext e : ctxs) {
			r[pos++] = (IASTtypedNamedLambda) e.node;
		}
		return r;
	}
	...
	protected IASTtypedVariable[] toVariables(List<Token> vars, IType[] types) {
		...
		IASTtypedVariable[] r = new IASTtypedVariable[vars.size()]; // +
																	// (addSelf
																	// ? 1 :
																	// 0)];
		int pos = 0;
